...
Google Calendar integration displaying upcoming events with five layout variants.
...
```
gmail-calendar/
├── manifest.json                         # Module metadata, OAuth config, properties
├── connector/
│   └── index.js                          # Google Calendar API connector (OAuth)
│   └── index.test.ts                     # Connector tests
├── visualizations/
│   └── calendar/
│       ├── index.tsx                     # Layout switcher (XL/L/M/S/Vertical)
│       ├── CalendarXL.tsx                # 7-day full layout
│       ├── CalendarL.tsx                 # 7-day compact layout
│       ├── CalendarM.tsx                 # 3-day layout
│       ├── CalendarS.tsx                 # Today-only layout
│       ├── CalendarVertical.tsx          # 7-day row layout
│       ├── utils.ts                     # Date/event helpers
│       ├── utils.test.ts                # Utils tests
│       ├── index.test.tsx               # Visualization tests
│       └── style.css
└── README.md
```

This is a **hybrid module** with OAuth (Google Calendar API).
...
if (!sdk.oauth.isAuthorized()) {
  sdk.emit('hubble-google-calendar:data', { error: 'Not authorized' });
  return;
}
...
// Your connector MUST emit to that exact topic: sdk.emit('hubble-google-calendar:data', ...)
// If you emit to a different topic (e.g. ':state'), pass args explicitly:
//   useConnectorData<MyData>('hubble-google-calendar', 'hubble-google-calendar:state')
